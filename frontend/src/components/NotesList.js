...
export default function NotesList() {
    ...
    function updateList() {
        var json, size;
        axios.get("http://abrams.seclab.cs.ucsb.edu:31007/api/paste").then((res) => {
    ...
    function saveNote(id, newTitle, newContent) {
        ...
        axios.post(`http://abrams.seclab.cs.ucsb.edu:31007/api/paste/${id}/${t}/${c}`, {
            'id': id,
            'title': t,
            'body': c
        }).then(res => {
    ...
    function deleteNote(id) {
        axios.delete(`http://abrams.seclab.cs.ucsb.edu:31007/api/paste/${id}`).catch((err) => {
            console.log(err)
        }).finally(
            updateList()
        )
